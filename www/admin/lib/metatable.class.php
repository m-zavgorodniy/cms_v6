			...
			while ($row = mysqli_fetch_assoc($rs_field)) {
				if (!$this->is_list or $row['is_in_subquery'] or $row['is_title'] or $this->is_front) {
					...
					if ($this->is_list) {
						...
						// ',' == $matches[2] ? 1 : 0.. - needs one more character length for decimal point
						if (!$this->is_front and preg_match("/\((\d+?)(\,|\))/", $row['type'], $matches)) {
							if ((int)$matches[1]) $field_meta['width'] = (int)$matches[1] + (',' == $matches[2] ? 1 : 0);
						}
					}
			...
			if (!$this->id) { // added
				...
				if (!empty($this->sql_order)) {
					...
					foreach($subquery_order as $order_field) {
						if ('lookup' == $this->record_meta[$order_field]['type']) {
							$order_by_field = "`" . $order_field . "_lookup`";
						} else {
							$order_by_field = "`" . $this->table_meta['table_name'] . "`.`" . $order_field . "`";
						}
						$res .= $order_by_field . ($this->record_meta[$order_field]['order_desc']?" DESC":"") . ",";
					}
					unset($order_field);
					$res = rtrim($res, ','); // remove trailing comma
				} else {
...
/*		$this->records = array();
		...
		if (!isset($this->is_new) or !$this->is_new) { // is_new is set if type of object is Editor (?)
			// todo!!!
...
//				$this->rec_count = count($this->records);
	...
	*/
	...
	function __construct($table, $site_id, $id = NULL, $is_list = false, $sql_filter = NULL, $master_table = NULL, $master_id = NULL, $many2many_table = NULL, $records_from = NULL, $records_on_page = NULL, $conn = NULL, $front_lang_id = '') {
		if (!$conn) {
