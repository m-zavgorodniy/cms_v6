...
//	var $custom_templates;
	...
	function __construct($section_path, $section_id, $section_type, $site_id, $site_path_mode, $lang_id, $is_ajax_request, $conn, $config) {
		...
		$this->is_https = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == 'on');
...
//		$this->is_index_page = ('index' == $this->section_type);
			...
			if ($this->config['SEO_ENABLED']) {
				...
				"SELECT title, meta_keywords, meta_description, h1, body, redirect_url
					...
					WHERE ('" . mysqli_real_escape_string($this->conn, $_SERVER["REQUEST_URI"]) . "' REGEXP CONCAT('^', REPLACE(REPLACE(url, '?', '\\\\?'), '*', '[^\/]+'), '$')
							OR '" . mysqli_real_escape_string($this->conn, ($this->is_https ? 'https://' : 'http://') . $_SERVER["SERVER_NAME"] . $_SERVER["REQUEST_URI"]) . "' REGEXP CONCAT('^', REPLACE(REPLACE(url, '?', '\\\\?'), '*', '[^\/]+'), '$'))
						AND published <> 0
					ORDER BY url DESC
					LIMIT 1", $this->conn);
	...
	function get_site() {
		$rs = db_mysql_query("SELECT path, own_domain, locale, title FROM meta_site WHERE id = '" . $this->site_id . "'", $this->conn); // , custom_templates
		// SELECT path, own_domain, lang_id.. - lang_id is removed
		// supposed to be one record
		$row = mysqli_fetch_assoc($rs);
		mysqli_free_result($rs);
		if ($row) {
			$this->site_title = $row['title'];
...
//				$menu_item['url'] =  $this->site_path . $this->lang_path . $row['path'] . $row['dir'] . ($submenu_level == 1?'/' . $row['submenu_dir']:'') . ($submenu_level == 2?'/' . $row['submenu2_dir']:'') . '/';
				$menu_item['url'] = $this->site_path . $this->lang_path . $row['path'] . $row['dir'] . ($submenu_level >= 1?'/' . $row['submenu_dir']:'') . ((isset($row['submenu2_dir']) and $row['submenu2_dir'] and ($submenu_level == 1 and $row['submenu_section_type_id'] == 'menuitem' or $submenu_level == 2))?'/' . $row['submenu2_dir']:'') . ($row['dir']?'/':'');
			} else {
				$menu_item['url'] = $row['url'];
			}
			$menu_item['target_blank'] = $row['target_blank'];
		} else if (isset($row['submenu_id']) and $row['submenu_id'] and $row['submenu_section_type_id'] != 'menuitem') {
		...
		} else if (isset($row['submenu2_id']) and $row['submenu2_id'] and $row['submenu2_section_type_id'] != 'menuitem') {
			...
			$rs = db_mysql_query("SELECT id, section_type_id, path, dir, url, section_id, target_blank
								  FROM section
								  WHERE published <> 0 AND section_id = " . ((isset($row['submenu2_id']) and $row['submenu2_id'])?$row['submenu2_id']:((isset($row['submenu_id']) and $row['submenu_id'])?$row['submenu_id']:$row['id'])) . " ORDER BY sort_num", $this->conn);
