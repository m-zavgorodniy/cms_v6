...
if ($site = get_site(SITE_PATH, $conn)) {
	...
	if ($section = get_section_by_path(SITE_ID, SITE_PATH_MODE, $uri_path, $config['SEO_ENABLED'], $conn)) {
		...
		//	$real_url = $section['path_full'];
			$uri_path_expl = explode($section['path_full'], $uri_path, 2);
			$url_params_str = '/' . trim(next($uri_path_expl), '/') . '/'; // (SITE_PATH_MODE?SITE_PATH:'') . $section['path_full']
		...
		/////}
			if ($sef_url_rules = get_sef_url_rules($conn, SECTION_TYPE)) { 
				...
				/*			$real_url_pretender = array_shift($url_exp);
						...
						} else if (!empty($url_params_exp)) {
							// $sef_url_rules is ordered by order_num
							$url_params = array($url_params_exp[$param_rule['order_num']]);
						}
						if (isset($url_params) and !isset($_GET[$param_rule['real_param_name']])) { // if the same param is in the query string and in the rewrited path the query param is stronger, it stays in _GET
					...
					/*							$rs = db_mysql_query("SELECT id, `" . $param_rule['values_source_field'] ."`" . (($param_rule['multi_lang_field'] and $config['SEO_URL_SOURCE_LANG_ID'] != '')?", `" . current(explode('_' . $config['SEO_URL_SOURCE_LANG_ID'], $param_rule['values_source_field'])) ."`":'') . " FROM `" . $param_rule['values_table'] . "` WHERE `" . $param_rule['values_target_field'] ."` = '" . mysqli_real_escape_string($conn, $real_param_value, ) . "'", $conn);
												...
												while ($row = mysqli_fetch_row($rs)) {
													if (0 === strpos($row[0], $param_rule['values_source_field_name'] . '_')) {#
														$exp_row = explode('_', $row[0]);
														$extra_langs[] = end($exp_row);
