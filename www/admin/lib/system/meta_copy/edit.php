		...
		if (true === ($r = db_mysql_query($query, $this->conn))) {
			...
			while ($row = mysqli_fetch_assoc($rs)) {
				...
				foreach($row as $field => $val) {
					if (in_array($field, array('type_extra', 'lookup_meta_table_field_id', 'lookup_external_meta_table_id', 'comment', 'default_value', 'default_order_num', 'in_subquery_colnum', 'in_subquery_title', 'sql_value', 'unit_id')) and $val == '') {
						$values[] = "NULL";
					} else {
				...
				unset($val);
				$query = "insert into meta_table_field (field, meta_table_id, title, title_en, meta_table_field_group_id, type, type_extra, lookup_multi, nullable, lookup_meta_table_field_id, lookup_external_meta_table_id, lookup_filter, required, sort_num, comment, default_value, published, readonly, is_title, is_in_subquery, in_subquery_wide, default_order_num, in_subquery_colnum, in_subquery_title, editable, multi_lang, sql_value, unit_id) VALUES (" . implode(",", $values) . ")";
				if (true !== ($r = db_mysql_query($query, $this->conn))) break;

				$new_field_id = current(mysqli_fetch_row(db_mysql_query("SELECT LAST_INSERT_ID()", $this->conn)));
