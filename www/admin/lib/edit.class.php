					...
					foreach($this->record_meta as $field => $meta) {
						...
						<?	} ?>
							...
							<?	if ($meta['lookup_filter']) {
									...
									$input_meta_lookup_filter = $meta['lookup_filter'];
									$meta['lookup_filter'] = preg_replace_callback("/{([a-zA-Z0-9_\-]+)}/U",
										function($matches) {
										    foreach($matches as $match) {
										        return $this->init_params[$match];
										    }
										},
										$input_meta_lookup_filter
									);
								}
								if ($meta['default']) {
...
//					$_SESSION["lastInsertID"] = $this->input_params['id'];
	...
	function on_update_success() { ?>
		...
		<?	if (!$_GET['quickadd']) {
				...
				<? }
				if ($this->close_on_update_success) { ?>
					window.close();
			<?	}
			} else { ?>
//				window.opener.location.reload();
...
<?	}
	...
	function set_on_update_success_close($val) {
		$this->close_on_update_success = $val;
	}
	...
	function lookup($meta_table_name, $field_name, $site_id, $current_id, $sql_lookup_filter, $conn, $sql_custom = NULL, $options_custom = NULL, $is_multi = false, $master_field_name = NULL, $is_external = false) {
		// todo! or do something. or hard code this for section dunno
		$tree_lookup_table_names = array('section');
...
/*						if (false !== mysqli_query($conn, "SELECT sort_num FROM `" . $table_name . "` WHERE 1 LIMIT 1")) {
						...
						// todo. why test against $sql_custom is in here (see if ($sql_custom) .. above)
...
/*			// gonna put column's default value into the NULL columns only
	...
	function Editor($meta_table, $site_id, $id, $get_params, $post_params) {
		...
		parent::__construct($meta_table, $site_id, $id);

		$this->new_param_name = 'new';
		$this->update_param_name = '__update';
		$this->close_on_update_success = true;
