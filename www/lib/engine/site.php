...
/*	if (!defined('SECTION_ID')) {
	...
	define('IS_AJAX_REQUEST', isset($_GET['is_ajax']) or isset($_GET['ajax_inner']) or isset($_SERVER['HTTP_X_REQUESTED_WITH']) and strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest');
...
/*			// !! todo
	...
	if (!$http_error) { 
		...
		if ($single_item_title) {
			...
			if (isset($PAGE_TITLES) and (!is_array($PAGE_TITLES) or false === array_search($single_item_title, $PAGE_TITLES[$site->lang_id]))) {
...
//	define('DATA_SINGLE_PARAM_TAG_NAME', 'single_param_name');
	...
	if ($data_tables) {
		$g_page = (isset($_GET[$config['DATA_PAGE_PARAM_NAME']]) and (int)$_GET[$config['DATA_PAGE_PARAM_NAME']])?(int)$_GET[$config['DATA_PAGE_PARAM_NAME']]:1;
		$has_data_sub = false;

		foreach ($data_tables as $table => $meta) {
			if (isset($meta['id_param_name']) and isset($_GET[$meta['id_param_name']]) and 0 != (int)$_GET[$meta['id_param_name']]) {
			...
			if ('news_folder' == $table or 'doc_folder' == $table) {
				...
				if ($data->init()) {
					...
					$res[$table_detail][DATA_COUNT_TAG_NAME] = '';
...
//					$res[$table][DATA_SINGLE_PARAM_TAG_NAME] = $meta['id_param_name'];
	...
	// news title has priority 
	if (isset($news_item_title)) {
		$single_item_title = $news_item_title;
	} else if (isset($record_item_title)) {
		$single_item_title = $record_item_title;
	}
...
function data_output_filter(&$records, $field_types, $locale) {
	...
	foreach ($records as $record) {
		foreach ($record as $field => $value) {
			if (!is_array($value) and isset($field_types[$field])) {
