	...
	<?	$this->extra_data_section_types = array();
		...
		while ($row = mysqli_fetch_assoc($rs_section)) { //?>
			<div class="cm-container">
				<a href="viewlist.php?type=top_section&id=<?=$row['id']?>" target="content" onclick="window.location.href='doclist.php?type=container&mode=sitetree&view=<?=$row['id']?>#h_section_<?=$row['id']?>';setCurrent(this)" id="section_<?=$row['id']?>"><span><img src="images/icons/bullet_toggle_<?=($row['id']==$g_view or !isset($g_view))?'minus':'plus'?>.png" alt=""></span><?=htmlspecialchars($row['title'])?></a>
			</div>
